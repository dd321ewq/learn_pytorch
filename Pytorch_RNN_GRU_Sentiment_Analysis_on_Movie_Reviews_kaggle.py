...
class NameDataset(Dataset):
    def __init__(self,is_train_set=True):
        filename = '/kaggle/input/movies/movie_train.tsv' if is_train_set else '/kaggle/input/movies/movie_val.tsv'
...
class RNNClassifier(torch.nn.Module):
    ...
    def forward(self,input,seq_lengths):
        ...
        # pack them up
            ...
            gru_input = pack_padded_sequence(embedding,seq_lengths)  ### make be sorted by descendent  打包变长序列    lengths' argument should be a 1D CPU int64 tensor, but got 1D cuda:0 Long tensor
...
###################################################################################################  最终验证模块
class sampleSubmissionDataset(Dataset):
    def __init__(self):
        filename = '/kaggle/input/movies/movie_test.tsv'
        data = pd.read_csv(filename,sep='\t')

        self.reviews = data['Phrase'].tolist()
        self.len = len(self.reviews)  #长度
...
sampleSubmissionset = sampleSubmissionDataset()
sampleSubmissionsetloader = DataLoader(sampleSubmissionset, batch_size=BATCH_SIZE, shuffle=False)

def make_tensors2(names):
    sequences_and_lengths = [name2list(name) for name in names]
    name_sequences = [sl[0] for sl in sequences_and_lengths]  #名字的ascll值
    seq_lengths = torch.LongTensor([sl[1] for sl in sequences_and_lengths]) #单独把列表长度拿出来 （名字的长度）
...
def sampleSubmissionModel(val_list):
    print("sampleSubmissionModel...")
    with torch.no_grad():
        for i,(reviews) in enumerate(sampleSubmissionsetloader,1):
            inputs,seq_lengths = make_tensors2(reviews)
            output = classifier(inputs,seq_lengths)
            pred=output.max(dim = 1,keepdim=True)[1]
...
if __name__=='__main__':
    ...
    VAL=True
    sampleSubmissionModel(val_list)
    for i in range(0,20):
        print(val_list[i])
    with open("/kaggle/working/output_100_2.txt", "w",encoding="utf-8") as file:
        for num in val_list:
            file.write(str(num))
